# laravel-guard advanced app

Production-shaped guarded app: env-driven `SecurityConfig` engine tuning
(`src/Config.php`), a router whose admin routes drive the engine's ban manager
...
image published to `ghcr.io/rennf93/laravel-guard-example` by the repo's
`container-release` workflow.

Layout:

- `public/index.php`: app assembly (engine, guard, request handling)
- `src/Config.php`: env-driven `SecurityConfig` plus the admin gate closure
- `src/Routes.php`: guarded router (Illuminate request in, response out)
...
| `REDIS_PREFIX` | `guard_core_laravel:` | Key namespace for bans and rate limits |
...
4. `GET /` with `X-Forwarded-For: 203.0.113.7` (trusted compose network resolves it) is blocked `403` with body `Blocked by laravel-guard`

# laravel-guard simple app

Minimal guarded app: a single `public/index.php` that wraps a closure router
with `RenzoFranceschini\GuardCoreLaravel\GuardMiddleware` (the laravel-guard
...
The Illuminate components are used as a minimal bootstrap (no full Laravel
skeleton): `Request::capture()` for input, `Illuminate\Http\Response` for
output, `Response::send()` for emission. In a full Laravel application the
same middleware registers in the HTTP kernel or `bootstrap/app.php` middleware
stack; the engine and its `SecurityConfig` are identical.
...
- `customErrorResponses`: `403` body is `Blocked by laravel-guard`
- `excludePaths`: `/health`
...
4. XSS query param (`?q=<script>alert(1)</script>`) trips the `xss` threat ban (threshold 1): `403` with body `Blocked by laravel-guard`
5. The same IP is now Redis-banned: `GET /` is blocked `403` with body `Blocked by laravel-guard`
